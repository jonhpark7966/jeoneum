...
## ⚠️ WIP — 작업 보류 (2026-06-02)

이 프로젝트는 현재 **보류(WIP)** 상태입니다. 엔드투엔드 파이프라인(CLI · REST · WebUI · Docker)은 *기능적으로* 동작하지만, **더빙 음성 품질이 실사용 기준에 미달**하여 여기서 일단락합니다.

### 마지막 테스트 결과 — 품질 미달 판정
- **1인 발화 / 단일 화자** (본인 레퍼런스 오디오, `out_extra1`): 한국인 화자의 레퍼런스로 영어를 합성한 결과 **중국어 억양이 섞인 영어 음성**이 나옴. 크로스링구얼 보이스 클론의 억양·명료도 품질이 부족.
- **2인 대화**: 결과가 **완전히 붕괴**. 화자 분리와 화자별 보이스 매칭이 신뢰성 있게 동작하지 않아 사실상 사용 불가.

### 재작업 방향 (재개 시)
현 구조로는 다화자 더빙을 신뢰성 있게 처리할 수 없어 **처음부터 다시 설계**가 필요합니다. 최소한 다음이 필요합니다:
1. 분리된 **각 화자에 레퍼런스 오디오를 직접 매칭하는 UI**.
...
> ⏸️ **Status: 보류(WIP).** CLI · REST API · WebUI · Docker 서빙까지 **기능 구현은 완료**되어 파이프라인이 끝까지 동작하지만, **더빙 음성 품질이 실사용 기준에 미달**해 작업을 일단락했습니다(위 **"⚠️ WIP — 작업 보류"** 섹션 참조). 아래의 "동작" 설명은 **기능 동작을 의미하며 출력 품질을 보증하지 않습니다.**
> 상세 설계 [`docs/spec.md`](docs/spec.md), 사용법 [`docs/usage.md`](docs/usage.md).

## 구현 상태 (2026-06-02 기준 · 보류)
...
- **크로스링구얼 보이스 클론 품질 (보류 사유, 치명적)** — 깨끗한 본인 레퍼런스 오디오를 써도 한국인 화자 → 영어 합성 시 **중국어 억양이 섞인 영어**가 생성됨(1인 테스트 `out_extra1`). 원본(타 언어·배경 섞임)에서 추출한 ref는 더 나빠 라운드트립 ASR에서 한/영 혼동으로 인식됨. **현 TTS 엔진의 크로스링구얼 클론 품질이 실사용 기준 미달** — 핵심 병목.
- **다화자 더빙 붕괴 (보류 사유, 치명적)** — 2인 대화 테스트가 **완전히 붕괴**. 화자 분리 → 화자별 ref 추출 → 보이스 매칭 경로가 신뢰성 있게 동작하지 않음. 화자별 레퍼런스 매칭 UI/API 부재가 근본 원인 → **재설계 필요**.
